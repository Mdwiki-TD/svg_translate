...
@bp_main.post("/")
def start():
    require_oauth = current_app.config.get("USE_MW_OAUTH", False)
    user = current_user()
    if require_oauth and not user:
        session["post_login_redirect"] = request.referrer or url_for("main.index")
